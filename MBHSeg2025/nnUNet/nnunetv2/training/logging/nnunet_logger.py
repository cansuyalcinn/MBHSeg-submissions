...
class nnUNetLoggerCBS(object):
    ...
    def plot_progress_png(self, output_folder):
        ...
        epoch = min([len(i) for i in self.my_fantastic_logging.values()]) - 1  # lists of epoch 0 have len 1
        sns.set(font_scale=2.5)
        fig, ax_all = plt.subplots(6, 1, figsize=(40, 90))  # <-- 4 subplots now
        # regular progress.png as we are used to from previous nnU-Net versions
        ax = ax_all[0]
        ax2 = ax.twinx()
        x_values = list(range(epoch + 1))
        ...
        # CANSU: ADDED Consistency weight
        if 'consistency_weight' in self.my_fantastic_logging:
            ax = ax_all[3]
            ax.plot(x_values, self.my_fantastic_logging['consistency_weight'][:epoch + 1], 
                    color='m', ls='-', label="consistency weight", linewidth=4)
            ax.set_xlabel("epoch")
            ax.set_ylabel("consistency weight")
            ax.legend(loc=(0, 1))

        # CANSU: ADDED mutual learning  loss
        if 'loss_ml' in self.my_fantastic_logging:
            ax = ax_all[4]
            ax.plot(x_values, self.my_fantastic_logging['loss_ml'][:epoch + 1], 
                    color='g', ls='-', label="loss_ml", linewidth=4)
            ax.set_xlabel("epoch")
            ax.set_ylabel("loss_ml")
            ax.legend(loc=(0, 1))

        # CANSU: ADDED weighted loss
        if 'weighted_loss_ml' in self.my_fantastic_logging:
            ax = ax_all[5]
            ax.plot(x_values, self.my_fantastic_logging['weighted_loss_ml'][:epoch + 1], 
                    color='b', ls='-', label="weighted_loss_ml", linewidth=4)
            ax.set_xlabel("epoch")
            ax.set_ylabel("weighted_loss_ml")
            ax.legend(loc=(0, 1))

        # CANSU: ADDED classification loss
        if 'loss_cls' in self.my_fantastic_logging:
            ax = ax_all[5]
            ax.plot(x_values, self.my_fantastic_logging['loss_cls'][:epoch + 1], 
                    color='b', ls='-', label="loss_cls", linewidth=4)
            ax.set_xlabel("epoch")
            ax.set_ylabel("loss_cls")
            ax.legend(loc=(0, 1))
