...
class nnUNetTrainerV2_SSL(nnUNetTrainerV2):
    ...
    def do_split(self):
        ...
        # print("tr_labeled_keys: ", tr_labeled_keys)
        print("tr labeled keys len: ", len(tr_labeled_keys))
        # print("tr_unlabeled_keys: ", tr_unlabeled_keys)
        print("tr unlabeled keys len: ", len(tr_unlabeled_keys))
    ...
    def _labeled_unlabeled_filter(self, sorted_datasets):
        ...
        for identifier in sorted_datasets:
            # Extract the numeric part of the identifier, assuming 'mbh_id' is followed by the number.
            num_part = int(identifier.split('_')[-1])

            if 0 <= num_part <= 191:
                labeled_keys.append(identifier)
            elif 192 <= num_part <=  753: # test wit with smaller 192 753 -- 84 unlabeled (192 to 275)
                unlabeled_keys.append(identifier)

        return labeled_keys, unlabeled_keys
